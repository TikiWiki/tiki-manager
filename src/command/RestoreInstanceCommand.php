...
class RestoreInstanceCommand extends Command
{
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$instances = TrimHelper::getInstances('no-tiki');
		$instancesInfo = TrimHelper::getInstancesInfo($instances);

		$restorableInstances = TrimHelper::getInstances('restore');
		$restorableInstancesInfo = TrimHelper::getInstancesInfo($restorableInstances);
		...
		if (isset($instancesInfo) && isset($restorableInstancesInfo)) {
			...
			$question = TrimHelper::getQuestion('Which instance(s) do you want to restore to', null, '?');
			...
			foreach ($selectedInstances as $instance) {
				...
				$io->newLine();
				$renderResult = TrimHelper::renderInstancesTable($output, $restorableInstancesInfo);

				$question = TrimHelper::getQuestion('Which instance do you want to restore from', null, '?');
				$question->setValidator(function ($answer) use ($restorableInstances) {
					return TrimHelper::validateInstanceSelection($answer, $restorableInstances);
				});
				...
				$question = TrimHelper::getQuestion('Which backup do you want to restore', null, '?');
